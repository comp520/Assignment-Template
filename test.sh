		...
		then
			...
			do
				...
				then
					...
					if [ $LOG -eq 1 ]
					then
						echo "$TEST: $OUTPUT [$STATUS_TEXT]" >> ${PHASE}_${TYPE}.log
					fi
				fi
			done                                 
			if [ $VERBOSE -eq 1 ]
			...
			echo -e "\e[${STATUS_COLOUR}m# ${PHASE} ${TYPE}: ${COUNT_PASSED}/${COUNT}\e[49m"
			RESULTS+=("\e[${STATUS_COLOUR}m# ${PHASE} ${TYPE}: ${COUNT_PASSED}/${COUNT}\e[49m")
		fi
	done
done
